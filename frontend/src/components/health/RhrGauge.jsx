...
import { RHR_SCALE, getRhrZone } from '../../utils/rhrGuides'

const ARC_LENGTH = 251.2

function gaugeAngle(ratio) {
  if (ratio == null) return 0
  const clamped = Math.min(Math.max(Number(ratio), RHR_SCALE.min), RHR_SCALE.max)
  return ((clamped - RHR_SCALE.min) / (RHR_SCALE.max - RHR_SCALE.min)) * 180
}
...
export default function RhrGauge({ ratio, today, usual, title = 'Vs usual' }) {
  const zone = getRhrZone(ratio, today, usual)
  const dashLength = (gaugeAngle(ratio) / 180) * ARC_LENGTH
  const color = zone.color || '#FB7185'
