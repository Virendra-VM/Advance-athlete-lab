import { HRV_SCALE, HRV_ZONES, hrvMarkerPercent, getHrvZone } from '../../utils/hrvGuides'

export default function HrvZoneStrip({ ratio, assessment, activeId, onSelect, zones = HRV_ZONES }) {
  const marker = hrvMarkerPercent(ratio)
  const current = activeId || getHrvZone(ratio, assessment).id
  const span = HRV_SCALE.max - HRV_SCALE.min
