...
import { DAILY_SCALE, getDailyZone } from '../../utils/dailyGuides'

const ARC_LENGTH = 251.2

function gaugeAngle(ratio) {
  if (ratio == null) return 0
  const clamped = Math.min(Math.max(Number(ratio), DAILY_SCALE.min), DAILY_SCALE.max)
  return ((clamped - DAILY_SCALE.min) / (DAILY_SCALE.max - DAILY_SCALE.min)) * 180
}
...
export default function DailyGauge({ ratio, today, title = 'Vs usual' }) {
  const zone = getDailyZone(ratio, today)
  const dashLength = (gaugeAngle(ratio) / 180) * ARC_LENGTH
  const color = zone.color || '#5B8DEF'
