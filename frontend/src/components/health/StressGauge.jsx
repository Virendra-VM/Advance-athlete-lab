...
import { STRESS_SCALE, getStressZone } from '../../utils/stressGuides'

const ARC_LENGTH = 251.2

function gaugeAngle(ratio) {
  if (ratio == null) return 0
  const clamped = Math.min(Math.max(Number(ratio), STRESS_SCALE.min), STRESS_SCALE.max)
  return ((clamped - STRESS_SCALE.min) / (STRESS_SCALE.max - STRESS_SCALE.min)) * 180
}
...
export default function StressGauge({ ratio, today, title = 'Vs usual' }) {
